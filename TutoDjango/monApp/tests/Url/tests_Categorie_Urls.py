...
class CategorieUrlsTest(TestCase):
    ...
    def test_categorie_create_url_is_resolved(self):
        url = reverse('crt_categorie')
        self.assertEqual(resolve(url).view_name, 'crt_categorie')
        self.assertEqual(resolve(url).func.view_class, CategorieCreateView)

    def test_categorie_update_url_is_resolved(self):
        url = reverse('categorie_chng', args=[1])
        self.assertEqual(resolve(url).view_name, 'categorie_chng')
        self.assertEqual(resolve(url).func.view_class, CategorieUpdateView)

    def test_categorie_delete_url_is_resolved(self):
        url = reverse('dlt_categorie', args=[1])
        self.assertEqual(resolve(url).view_name, 'dlt_categorie')
    ...
    def test_categorie_create_response_code_OK(self):
        response = self.client.get(reverse('crt_categorie'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_categorie_update_response_code_OK(self):
        url = reverse('categorie_chng', args=[self.ctgr.idCat]) # idCat existant
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_categorie_update_response_code_KO(self):
        url = reverse('categorie_chng', args=[9999]) # idCat non existant
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_redirect_after_categorie_creation(self):
        response = self.client.post(reverse('crt_categorie'), {'nomCat': 'CategoriePourTestRedirectionCreation'})
    ...
    def test_redirect_after_categorie_updating(self):
        response = self.client.post(
            reverse('categorie_chng', args=[self.ctgr.idCat]),
